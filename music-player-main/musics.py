...
def playMusic():
    if(root.songAdded == False):
        root.screenMessage.set("First add some Music buddy!")
    else:
        try:
...
two.pack(fill=X)
three = Label(root, text="Created By ASharanya for projects", bg="green",fg="white")
three.pack(fill=X)
...
root.screenMessage.set("Welcome, to This Music Player")
